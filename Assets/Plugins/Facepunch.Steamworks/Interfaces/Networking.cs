...
namespace Facepunch.Steamworks
{
    public class Networking : IDisposable
    {
        ...
        public bool GetSessionState(ulong steamID, out SteamP2PSessionState state) {
            SteamNative.P2PSessionState_t pConnectState = new SteamNative.P2PSessionState_t();
            bool ret = networking.GetP2PSessionState(steamID, ref pConnectState);

            state = new SteamP2PSessionState();
            state.ConnectionActive = pConnectState.ConnectionActive;
            state.Connecting = pConnectState.Connecting;
            state.P2PSessionError = pConnectState.P2PSessionError;
            state.UsingRelay = pConnectState.UsingRelay;
            state.BytesQueuedForSend = pConnectState.BytesQueuedForSend;
            state.PacketsQueuedForSend = pConnectState.PacketsQueuedForSend;
            state.RemoteIP = pConnectState.RemoteIP;
            state.RemotePort = pConnectState.RemotePort;
            return ret;
        }


        public struct SteamP2PSessionState {
